...
#include <iostream>
	...
	void Scene::Draw(int shaderIndx,int cameraIndx,int buffer,bool toClear,bool debugMode)
	{
		...
		{
			if(shaderIndx>0)
				Clear(1,1,1,1); // from tirgul 9 fix the background color
			else
				Clear(0,0,0,0);
		}
	...
	// added here
	void Scene::CustonDraw(int shaderIndx, int cameraIndx, int buffer, bool toClear, bool debugMode, int screenNum)
	{
		glEnable(GL_DEPTH_TEST);
		glm::mat4 Normal = MakeTrans();
		...
		if (screenNum == 0)
		{
			glViewport(0, 256, 256, 256); // up left
		}
		else if (screenNum == 1)
		{
			glViewport(256, 256, 256, 256); // up right
		}
		else if (screenNum == 2)
		{
			glViewport(0, 0, 256, 256); // buttom left
		}
		else if (screenNum == 3)
		{
			glViewport(256, 0, 256, 256); // buttom right
		}
	...
	void Scene::MouseProccessing(int button)
	{
		...
		if(pickedShape == -1)
		{
			...
			else
			{
				...
				int y = -1;
				int z = 0;
				MyRotate(xrel / 2.0f, GetRot()*glm::vec3(x, y, z), 0);

				x = -1;
				y = 0;
				z = 0;
				MyRotate(yrel / 2.0f, GetRot()*glm::vec3(x, y, z), 0);
